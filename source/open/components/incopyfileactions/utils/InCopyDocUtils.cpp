...
//				flags = IOpenFileCmdData::kOpenCopy;
	...
	if (err == kSuccess) 
	{
		if (cmd->GetItemList() && cmd->GetItemList()->Length() > 0)
		{
          ...
          Utils<IGalleyUtils>()->AttachSpecialContainerToFrames(doc, model);
				}
			}
		...
		while(storyToImport != UIDRef::gNull && globalErr != kCancel)
		{
			...
			if (!fileMissing)
			{
				...
				if (link)
				{
					...
					{
						// dont want to bypass import linked story of INDD file in InCopy
						// Handled at MissingLinksOpenDocResponder 
						if (fileTypeID != kInDesignFileTypeInfoID)
						{
				...
				if (textModel)
				{
					...
					if (err == kSuccess)
					{
						...
						if (err == kSuccess) {
                            ...
                            if(Utils<Facade::IEndnoteFacade>()->IsEndnoteStory(storyToImport))
                            {
	...
	if ( link != nil )
	{
		...
		if (linkResource && Utils<IStoryUtils>()->IsAStoryFormatName( linkResource->GetFormatType() ))
		{
			...
			if( !wstr.empty() )
			{
				...
				// HFS2POSIX : Redirect to API in 20.0
