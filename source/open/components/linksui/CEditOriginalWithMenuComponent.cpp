			...
			else if (nAction == kEditOriginalOtherAppActionID)
			{
				...
				for(UIDList::const_iterator iter = targetLayoutLinks.begin(); iter != targetLayoutLinks.end(); ++iter)
				{
					...
					else
					{
                        ...
                        Utils<ICloudLibraryLinkedAssetPublicUtils> iCloudLibraryLinkedAssetPublicUtils;
						if (iCloudLibraryLinkedAssetPublicUtils &&
                            iCloudLibraryLinkedAssetPublicUtils->IsCloudScheme(uri))
						{
							action = kDisabled_Unselected;
							break;
						}
