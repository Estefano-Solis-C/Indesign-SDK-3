	...
	for (int32 resourceIndex = 0; resourceIndex < resources.Length(); resourceIndex++)
	{
		...
		for (int32 linkIndex = 0; linkIndex < links.Length(); linkIndex++)
		{
			...
			if (link->GetResource() == ::GetUID(resource))
			{
                ...
                {
                    std::string pLogStr = "";
                    if(!(appInfo.GetDisplayName()).IsEmpty())
                        pLogStr= "EditWith " + (appInfo.GetDisplayName()).GetUTF8String();
                    else
                        pLogStr= "EditWithOriginal";
                    usageTracking->LogUsageEvent(IDUsageTracking_AssetTransfer, IDUsageTracking_AssetTransferEditWith, pLogStr.c_str());
                }
				break; // next resource
			}
		}
