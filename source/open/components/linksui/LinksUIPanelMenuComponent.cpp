...
#include "ICloudDocsPrefs.h"
#include "ITargetUIDListForRelinkingContext.h"
// Includes:
...
private:
    ...
    bool16 GetXMPPacketInformation(const UIDRef& ref, std::string& packet) const;
};
...
enum WhichMenuItem { kRelinkMenuItemEnum, kUpdateMenuItemEnum, kEmbedMenuItemEnum, kUnEmbedMenuItemEnum, kUnlinkMenuItemEnum, kCCRelinkMenuItemEnum, kAALRelinkMenuItemEnum, kReplaceFPOWithOriginalMenuItemEnum, kCCACRelinkMenuItemEnum};
...
PMString GetDynamicMenuItemNameForAction(WhichMenuItem menuItem, bool applyToAllInstances, PMString& singleResourceName)
{
    if(applyToAllInstances)
    {
        ...
        else
        {
            ...
            {
                case kCCACRelinkMenuItemEnum:
                    menuItemName.SetCString("#RelinkAdobeCloudResource ^1 Menu");
                    break;
                case kCCRelinkMenuItemEnum:
    ...
    else
    {
        switch(menuItem)
        {
            ...
            case kCCACRelinkMenuItemEnum:
                return "#LinksUIRelinkFromCCACLinkMenu";
            case kCCRelinkMenuItemEnum:
    ...
    for (int32 i = 0; i < nCount; i++)
    {
        ...
        switch (nAction.Get())
        {
            ...
            case kLicenseStockAssetActionID:
            {
                if (targetLinks.size() == 1)
                {
                    ...
                    if (linkResource)
                    {
                        ...
                        Utils<ICloudLibraryLinkedAssetPublicUtils> cloudLibraryAsset;
                        if (cloudLibraryAsset != NULL && linkResource->GetStoreState() != ILinkResource::kEmbedded  && cloudLibraryAsset->IsCloudScheme(uri))
                        {
                            ...
                            if (cloudLibraryAsset->IsElementStock(elementRefStr) && !cloudLibraryAsset->IsElementStockLicensed(elementRefStr))
                                listToUpdate->SetNthActionState(i, kEnabledAction);
                        }
                    }
                }
            ...
            case kLinksUIUpdateLinkActionID :
            {
                ...
                break;
            case kLinksUIRelinkToCCACAssetActionID:
            case kLinksUIRelinkToCloudAssetActionID:
            {
                ...
                } else if (nAction == kLinksUIRelinkToCCACAssetActionID) {
                    listToUpdate->SetNthActionName(i, GetDynamicMenuItemNameForAction(kCCACRelinkMenuItemEnum, relinkingAllInstances, singleResourceToRelinkName));
                }
            }
                break;
            ...
            {
                bool16 addAALToMenuItem = Utils<IRelinkUtils>()->ShouldShowAALRelinkAction();
                if (!addAALToMenuItem)
                    break;
                PMString singleResourceToRelinkName;
                bool relinkingAllInstances = Utils<ILinksUIUtils>()->IsMenuActionEntirelyOnLinkResourceRows(widget);
                ...
                if(relinkingAllInstances)
                {
                    ...
                    if(nAction == kLinksUIRelinkFileExtensionActionID)
                    {
                        for(int resIndex = targetResources.Length() - 1; resIndex >= 0; --resIndex)
                        {
                            ...
                            bool isAdobeCloudScheme = false;
                            if (theLinkResource)
                            {
                                URI uri = theLinkResource->GetId().GetURI();
								
								isAdobeCloudScheme = Utils<ICloudDocsPublicUtils>()->IsAdobeCloudScheme(uri);
                            }
                ...
                else
                {
                    ...
                    if(nAction == kLinksUIRelinkFileExtensionActionID)
                    {
                        for(int linkIndex = targetLinks.Length() - 1; linkIndex >= 0; --linkIndex)
                        {
                            ...
                            InterfacePtr<ILinkResource> theLinkResource(::GetDataBase(theLink), theLink->GetResource(), UseDefaultIID());
                            bool isAdobeCloudScheme = false;
                            if (theLinkResource)
                            {
                                URI uri = theLinkResource->GetId().GetURI();
            ...
            case kLinksUIPanelFileXMPInfoActionID:
            {
                if(targetLinks.size() == 1)
                {
                    ...
                    std::string packet;
                    bool16 xmpFound = GetXMPPacketInformation(::GetUIDRef(linkResource), packet);
    ...
    switch (actionID.Get())
    {
        ...
        case kLicenseStockAssetActionID:
        {
            ...
            if (linkResource)
            {
                ...
                Utils<ICloudLibraryLinkedAssetPublicUtils> cloudLibraryAsset;
                if (cloudLibraryAsset != NULL && cloudLibraryAsset->IsCloudScheme(linkURI))
                {
                    PMString elementRef(linkURI.GetComponent(URI::kPath));
                    std::string elementRefStr(elementRef.GetUTF8String());
                    if (cloudLibraryAsset->IsElementStock(elementRefStr) && !cloudLibraryAsset->IsElementStockLicensed(elementRefStr))
                    {
                        cloudLibraryAsset->LicenseStockElement(elementRefStr);
        ...
        case kEditOriginalLayoutSelectionActionID :
        {
            ...
            Utils<IDataLoggingUtils> dataLogging;
            dataLogging->SetkDesignLibrariesEditOriginalData(IDataLoggingUtils::kMenuEnum);
            Utils<ILinksUIUtils>()->EditAllOriginals(targetLayoutLinks, targetLayoutResources);
            dataLogging->ResetkDesignLibrariesEditOriginalData();
        }
            break;
        case kLinksUIEditOriginalActionID:
        {
            ...
            LogCloudAssetActions(actionID, targetLinks);
            Utils<IDataLoggingUtils> dataLogging;
            dataLogging->SetkDesignLibrariesEditOriginalData(IDataLoggingUtils::kLinksPanelEnum);
        ...
        case kLinksUIRelinkActionID:
        {
            if(Utils<ILinksUIUtils>()->IsMenuActionEntirelyOnLinkResourceRows(widget))
            {
                ...
                //Set the context first and corresponding caller will work on this context only
                InterfacePtr<ITargetUIDListForRelinkingContext> libraryContext(GetExecutionContextSession(), UseDefaultIID());
                if (libraryContext)
                {
                    libraryContext->SetRelinkElementReceivedContext(allResources, true);
                }
                InterfacePtr<ICloudDocsPrefs> cloudDocsPrefs((ICloudDocsPrefs*)::QuerySessionPreferences(IID_ICLOUDDOCSPREFS));
                if (cloudDocsPrefs) {
                    ICloudDocsPrefs::defaultType defaultOpenValue = cloudDocsPrefs->GetOpenDefaultValue();
                    bool isEnablePlaceLinks = true;
                    // in case cloud docs is disabled we will reset the pref to open native only

                    if(defaultOpenValue == ICloudDocsPrefs::defaultType::kCDP && isEnablePlaceLinks)
                    {
                        Utils<IRelinkUtils>()->DoRelinkSelectedLinkResourcesWithCCACAssets(allResources, kIDLinkClientID);
                    }
                    else
                    {
            ...
            else
            {
                ...
                if (libraryContext)
                {
                    libraryContext->SetRelinkElementReceivedContext(targetLinks, false);
                }
                UIDList newLinkUIDs(targetLinks.GetDataBase());
                InterfacePtr<ICloudDocsPrefs> cloudDocsPrefs((ICloudDocsPrefs*)::QuerySessionPreferences(IID_ICLOUDDOCSPREFS));
                if (cloudDocsPrefs)
                {
                    bool isEnablePlaceLinks = false;

                    ICloudDocsPrefs::defaultType defaultOpenValue = cloudDocsPrefs->GetOpenDefaultValue();
                    if(defaultOpenValue == ICloudDocsPrefs::defaultType::kCDP && isEnablePlaceLinks)
                    {
                        Utils<IRelinkUtils>()->DoRelinkSelectedLinksWithCCACAssets(targetLinks, kIDLinkClientID);
                    }
                    else
                    {
        ...
        case kRelinkAssetFromPropsPanelActionID:
        {
            ...
            UIDRef linkRef = selectedLinks.GetRef(0);
            UID linkUID(linkRef.GetUID());
            InterfacePtr<const ILink> oneLink(linkRef, UseDefaultIID());
            InterfacePtr<const ILinkResource> linkResource(targetLinks.GetDataBase(), oneLink->GetResource(), UseDefaultIID());
            bool isAdobeCloudScheme = false;
            if (linkResource)
            {
                URI uri = linkResource->GetId().GetURI();
                isAdobeCloudScheme = Utils<ICloudDocsPublicUtils>()->IsAdobeCloudScheme(uri);
            }
            if (isAdobeCloudScheme)
            {
                Utils<IRelinkUtils>()->DoRelinkSelectedLinksWithCCACAssets(selectedLinks, kIDLinkClientID);
            }
            else if (cloudLibraryAsset != NULL && cloudLibraryAsset->IsCloudLibraryAsset(selectedPageItems.GetRef(0)))
            {
...
void LinksUIPanelMenuComponent::LogCloudAssetActions(ActionID actionID, UIDList& targetLinks, bool16 unembedDone)
{
    ...
    if(db)
    {
        ...
        Utils<ICloudLibraryLinkedAssetPublicUtils> cloudLibraryAsset;
        if(cloudLibraryAsset != NULL && cloudLibraryAsset->IsCloudScheme(uri))
        {
            switch(actionID.Get())
            {
                case kRevealLinkInCloudLibrariesActionID:
...
bool16 LinksUIPanelMenuComponent::GetXMPPacketInformation(const UIDRef& ref, std::string& packet) const{
    bool16 xmpFound = kFalse;
    InterfacePtr<ILinkResource> iLinkResource(ref, UseDefaultIID());
    if (iLinkResource && iLinkResource->CanGetXMPPacketInfo())
        xmpFound = iLinkResource->GetXMPPacketInfo(packet);
    return xmpFound;
}
