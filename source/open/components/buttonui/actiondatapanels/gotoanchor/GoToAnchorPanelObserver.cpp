...
#if PLACELINKFORCLOUDDOCS_TODO
#error "Should be allowed for cloud docs too, once Place link is enabled in Cloud ID"
#endif
            if(doc->IsCloudDoc() && doc!=frontDoc)
                continue;
			...
			if ( !docSysFile )
			{
                IPubFile* pubFile=  doc->GetDocWorkSpace().GetDataBase()->GetPubFile();
                if(pubFile)
                {
                    docSysFile =pubFile->GetTempFile();
                    theAnchorFileList.push_back(IAnchorFileListData::AnchorFileData( FileUtils::SysFileToPMString( *docSysFile ), addName));
                }else{
                    //Adding a docSysFile to anchor list since for cloud docs pubfile doesn't exist 
                    docSysFile= new IDFile();
                    theAnchorFileList.push_back(IAnchorFileListData::AnchorFileData( FileUtils::SysFileToPMString( *docSysFile  ), addName));
...
IDocument* GoToAnchorPanelObserver::FindDocInDocList(IDFile &theSysFile)
{
	...
	if ( docList )
	{
		for (int i = 0; i < docList->GetDocCount(); i++)
		{
			...
			{
                IPubFile* pubFile=(::GetDataBase(docList->GetNthDoc(i))->GetPubFile());
                if(pubFile){
                    const IDFile *openfile2 = pubFile->GetTempFile();
                    if (openfile2 != nil )
                    {
