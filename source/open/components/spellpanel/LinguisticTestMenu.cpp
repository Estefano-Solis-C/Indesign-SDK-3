...
//			InterfacePtr<IWalkerScopeFactoryUtils>pScopeUtils(this, IID_IWALKERSCOPEFACTORYUTILS);
	...
	static void GuessScriptOfText()
	{
        InterfacePtr<IK2ServiceRegistry>    pServices(GetExecutionContextSession(), IID_IK2SERVICEREGISTRY);
        InterfacePtr<IK2ServiceProvider>pLinguMgrService( pServices->QueryDefaultServiceProvider( kLinguisticManagerService ) );
        InterfacePtr<IScriptOfText>scriptOfText( pLinguMgrService, IID_ISCRIPTOFTEXT );
        
		// simple Roman text is kCTRomanScript ...
		const char *romanText = "This is Roman text";
        
        bool16 isTextkCTRoman=scriptOfText->IsScriptRoman(romanText);
        ASSERT_MSG(isTextkCTRoman, "TestIsScriptOfText: simple Roman text." );

		// Roman text can be kCTJapaneseScript...
        bool16 isRomanTextkCTJapanese=scriptOfText->IsScriptRoman(romanText);
		ASSERT_MSG(isRomanTextkCTJapanese , "TestIsScriptOfText: Roman can be Japanese." );
        ...
        bool16 isHighAsciiTextkCTRoman=scriptOfText->IsScriptRoman(romanHighAscii);
        ASSERT_MSG(isHighAsciiTextkCTRoman, "TestIsScriptOfText: high ASCII Roman text." );

		// simple Japanese text is kCTJapaneseScript...
		const char *japaneseText = "Ç†Ç¢Ç§Ç¶Ç®";
        
        bool16 isTextkCTJapanese=scriptOfText->IsScriptJapanese(japaneseText);
		ASSERT_MSG(isTextkCTJapanese , "TestIsScriptOfText: simple Japanese text." );

		// Japanese text is not kCTRomanScript
        
        bool16 isJapaneseTextNotkCTRoman=!(scriptOfText->IsScriptJapanese(japaneseText));
		ASSERT_MSG(isJapaneseTextNotkCTRoman, "TestIsScriptOfText: Japanese is not Roman." );

		// mixed Japanese/Roman text is kCTJapaneseScript...
		const char *mixedText = "TextÇ†Ç¢Ç§Ç¶Ç®";
        
		bool16 isMixedTextkCTJapanese = scriptOfText->IsScriptJapanese(mixedText);
		ASSERT_MSG(isMixedTextkCTJapanese, "TestIsScriptOfText: mixed J/R text is Japanese." );
	}
