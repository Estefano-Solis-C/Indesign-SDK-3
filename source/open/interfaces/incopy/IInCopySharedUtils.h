...
class IInCopySharedUtils : public IPMUnknown
{
	public:
		...
		*/
        ...
        /**
            Helper that checks if a story is unlocked.  Returns kTrue unless all stories are locked (or no stories).
            @param db the database to check
            @return bool16 whether or not stories are unlocked.
         */
        virtual bool16 IsStoryUnlocked( IDataBase *db ) = 0;
