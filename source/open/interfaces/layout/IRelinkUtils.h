...
public:
	...
	*/
    ...
    /* Relink the passed in LinkResource with a CCAC asset by presenting the UAP panel in the mode where user can select an asset to relink to
    @param targetResources - a list of ILinkResource UIDs you want the user to relink.
    @param whichClientID - Link client identifier of the resources.
    @return - whether relink was successful or not.
    */
    virtual bool16 DoRelinkSelectedLinkResourcesWithCCACAssets(const UIDList& targetResources, LinkClientID whichClientID) const = 0;
	...
	*/
    ...
    /* Relink the passed in links with a CCAC asset by presenting UAP panel in the mode where user can select an asset to relink to.
    @param targetLinks - a list of ILink UIDs you want the user to relink.
    @param whichClientID - Link client identifier of the links.
@return - whether relink was successful or not.
    */
    virtual bool16 DoRelinkSelectedLinksWithCCACAssets(const UIDList& targetLinks, LinkClientID whichClientID) const = 0;
	...
	*/
    ...
    /* Relink the passed in link/linkResource with the CCAC asset which is represented by the elementRef passed as an argument.
    @param elementRef - unique identifier of the CCAC asset to relink to.
    @param link - the link which is to be relinked. If NULL, then the resource is to be relinked.
    @param linkResource - the resource which is to be relinked.
    @return - whether the relink was successful or not.
    */
    virtual bool16 DoRelinkWithCCACAsset(const std::string& elementRef, ILink* link, ILinkResource* linkResource) const = 0;
    ...
    /*
     Returns true, if RelinkUsingAALPanel option has to be shown in LinksUI, for internal purpose only
     */
    virtual bool16 ShouldShowAALRelinkAction() const = 0;
};
