...
public:
	...
	*/
    ...
    /** Returns the image name of the icon as a string by concatenating pluginID, rsrcType and iconRsrcID
     @param pluginID                 The plugin of the resource being considered.
     @param rsrcType                 The type of the resource , whether it is kPNGArtRsrcType etc.
     @param iconRsrcID               The icon resource id for the icon.
     returns image name for the icon
     */
    virtual std::string GetImageName(PluginID pluginID,RsrcType rsrcType,RsrcID rsrcID);
    
    /*
    Fallback logic which decides which icon is loaded based on various factors.
    @param pluginID                    The plugin of the resource being considered.
    @param iconRsrcID                  The icon resource ID for the icon.
    @param outRsrcType                 The type of the resource , whether it is kPNGArtRsrcType etc.
    @param outImageName                The image name of the icon set by the function
    @param iconScale                   The desired scaleFactor in which the icon is tried to be loaded.
    @param loadedIconScale             The actual scaleFactor for which the icon is loaded after executing the fallback logic.
    @param isDark                      Whether dark icon should be loaded.
    @return                            The icon resource is returned.
    */
    virtual IDResource LoadIconResource(PluginID pluginID, RsrcID iconRsrcID, RsrcType &outRsrcType, std::string &outImageName, float iconScale, float &loadedIconScale, bool16 isDark);
    
    /** Utility function that reads the preferences and returns the Bounding Box/Adornment/Handles scale factor set by the user.
        returns the bounding box scale factor.
     */
    virtual float GetBoundingBoxScaleFactor();
    
    /** Utility function that returns the current effective scale factor.
        This function returns the product of OS scale factor and the app scale factor.
        In case NULL is provided then it returns only the app scale factor in case of MAC.
        @param view - the input view for calculating the backing scale factor.
        returns the effective scale factor.
     */
    virtual PMReal GetEffectiveScaleFactor(const IControlView* view);
...
private:
...
#ifdef MACINTOSH
    void PrivateLayerDrawFocusRect(IControlView* view, const PMRect& r, bool16 flushDrawing);
    bool16 PrivateLayerDrawHiliteRect(IControlView* view, const PMRect& r, DrawType drawType, bool16 flushDrawing, PMReal hiliteThickness);
    bool16 PrivateLayerDrawHilitePath(IControlView* view, const PMPointListCollection& subPaths, DrawType drawType, bool16 flushDrawing, PMReal hiliteThickness);
    bool16 PrivateLayerDrawHilitePath(IControlView* view, SysWireframe hilitePath, bool16 flushDrawing, PMReal hiliteThickness);
    bool16 PrivateCopyBitsToLayer(IControlView* view, const Int32Rect& dstBounds, IDVPlatformOffscreen* srcOffscreen, const Int32Rect& srcBounds,
                                               const PMMatrix& clip2destMatrix, const SysRgn clipRgn);
#endif
    ...
    IDResource    LoadSVGIconResource(PluginID pluginID, RsrcID iconRsrcID, RsrcType &outRsrcType, std::string &outImageName, bool16 isDark=kFalse);
    IDResource    LoadPNGIconResource(PluginID pluginID, RsrcID rscID, RsrcID iconRsrcID, RsrcType &outRsrcType, std::string &outImageName,float iconScale, float &loadedIconScale, bool16 isDark=kFalse);

public:
	// For Internal use only
	virtual void RepositionAndShowContextUIPopup_Internal(IWindow* popupWindow,
		bool16 showWindow = kTrue,
		const PMRect& anchorRect = kZeroRect,
