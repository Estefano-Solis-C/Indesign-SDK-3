...
namespace os_utils
{
	...
	*/
	...
	/**
	 * Get the product of UI scale factor and OS scale factor for the given control view.
	 */
    DV_WIDGET_DECL float GetEffectiveScaleFactor(const IControlView* view);
#ifdef MACINTOSH
	/**
	 * Gets the OS scale factor only for Mac.
	 */
    DV_WIDGET_DECL float GetOSScaleFactor(const IControlView* view);
#endif
};
};
...
/** Returns whether the given childView is a child of the parentView - uses UI_Node API directly. */
DV_WIDGET_DECL bool IsChildOf(const IControlView* childView, const IControlView* parentView);
...
//Draw Colored dot on button
DV_WIDGET_DECL void DVDrawColorDotOnButton(dvaui::drawbot::Drawbot* drawbotP, dvaui::controls::UI_TextButton* textButton, const PMReal startX, const PMReal startY, const PMReal radius);
