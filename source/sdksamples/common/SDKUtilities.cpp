...
static const char k_MACDELCHAR_HFS = ':';
static const char k_MACDELCHAR = (SDKFileHelper::isMacPosixPathEnabled()) ? k_UNIXDELCHAR : k_MACDELCHAR_HFS;
static const char k_WINDELCHAR = '\\';
static const PMString k_UNIXDELIMETER ("/");
static const PMString k_WINDELIMETER("\\");
static const PMString k_MACDELIMETER ((SDKFileHelper::isMacPosixPathEnabled()) ? "/" : ":");
static const PMString k_UNIX_WINPARENTDIR ("..");
static const bool16 k_PosixEnabled = SDKFileHelper::isMacPosixPathEnabled();
static const PMString k_MACDELIMETER_HFS (":");
static const PMString k_MACPARENTDIR_HFS ("::");
static const PMString k_MACPARENTDIR = (k_PosixEnabled) ? k_UNIX_WINPARENTDIR : k_MACPARENTDIR_HFS;
...
{
	// the way I do this is if it has a mac(colon) or win delimeter then it can't be a unix path.
	return !(thisstring.Contains(k_WINDELIMETER)||(thisstring.Contains(k_MACDELIMETER) && !k_PosixEnabled) ||
			(thisstring.Contains(k_MACDELIMETER_HFS) && k_PosixEnabled));
}
...
{
	// the way I do this is if it has a unix(if we are using hfs path style) or win delimeter then it can't be a unix path.
	return !(thisstring.Contains(k_WINDELIMETER)||(thisstring.Contains(k_UNIXDELIMETER) && !k_PosixEnabled) ||
			(thisstring.Contains(k_MACDELIMETER_HFS) && k_PosixEnabled));
}
...
{
	// the way I do this is if it has a mac(colon, forwad-slash both are not expected in win path)
	// or unix delimeter then it can't be a win path
	// I also consider device names here...
	return !(thisstring.Contains(k_UNIXDELIMETER)||(thisstring.Contains(k_MACDELIMETER)) ||
			(k_PosixEnabled && thisstring.Contains(k_MACDELIMETER_HFS)));
}
...
bool SDKUtilities::IsAbsolutePath(PMString thisstring)
{
	...
	if (IsUnixPath(thisstring) && IsWinPath(thisstring)) // two out of three works for me
		return kFalse;
	...
	if ((IsWinPath(thisstring)) && (thisstring[0]==k_WINDELCHAR))
		return kTrue;
	if ((IsMacPath(thisstring)) && ((!k_PosixEnabled && thisstring[0]!=k_MACDELCHAR) // is it's hfs path, is must not start with colon
		|| (k_PosixEnabled && thisstring[0]==k_MACDELCHAR))) // if it's a posix path, it must start with forward-slash
		return kTrue;
	return kFalse;
}
...
void SDKUtilities::RemoveFirstElement(PMString& fromthis)
{
	...
	{	
		if (!k_PosixEnabled && fromthis[0] == k_MACDELCHAR) // if we are using hfs path style, we know we start with a "::"
		{
			fromthis.Remove(0,2);
			return;
		} 
...
void SDKUtilities::convertToMacPath(PMString& thisstring)
{
	...
	// if it's a posix path, we need not to do anythong
	if(!k_PosixEnabled) 
	{
	...
	for (int32 i = 0; i<thisstring.CharCount(); i++)
		if ((thisstring[i] == k_UNIXDELCHAR && !k_PosixEnabled) || (thisstring[i] == k_WINDELCHAR)
		|| (thisstring[i] == k_MACDELCHAR_HFS && k_PosixEnabled)) // want the mac delimeter
		{
			if(thisstring[i] != k_MACDELCHAR_HFS && k_PosixEnabled && i == 0)
			{
				thisstring.Remove(i);
...
void SDKUtilities::convertToWinPath(PMString& thisstring)
{
	...
	// If, it's a mac hfs path, replace all instances of :: with ..
	int32 position = thisstring.IndexOfString(k_MACPARENTDIR_HFS);
	while (position!=-1)
	{
		thisstring.Remove(position,2); // remove the "::"
		thisstring.Insert(k_UNIX_WINPARENTDIR,position);
		thisstring.Insert(k_WINDELCHAR,position+2); // replace it with "..\"
		position = thisstring.IndexOfString(k_MACPARENTDIR_HFS);
	}

	//  get all the MAC delimeters sorted.
	position = thisstring.IndexOfString(k_MACDELIMETER_HFS);
	while (position!=-1)
	{
