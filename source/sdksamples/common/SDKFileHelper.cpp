...
{
	enum PathStyle { 	kPOSIXPathStyle = 0, kHFSPathStyle, kWindowsPathStyle };
    static PathStyle pathStyle = kHFSPathStyle;
	static bool isPathStyleInitialized = false;
    if(isPathStyleInitialized == false)
    {
		isPathStyleInitialized = true;
        #ifdef MACINTOSH
            if(FileUtils::GetMacPathStyle() == kCFURLPOSIXPathStyle)
            {
                pathStyle = kPOSIXPathStyle;
            }
        #endif
            ...
            // Based on cookie
            IDFile cookiesPath;
            if (FileUtils::GetAppInstallationFolder(&cookiesPath))
            {
                if(cookiesPath.AppendName("Resources"))
                {
                    if(cookiesPath.AppendName("Cookies"))
                    {
                        if(cookiesPath.AppendName("ENABLE_POSIX_PATH"))
                        {
    ...
    return pathStyle == kPOSIXPathStyle;
}

const static PlatformChar k_SDKFileHelper_UNIXDELCHAR =('/');
const static PlatformChar  k_SDKFileHelper_MACDELCHAR_HFS = ':';
const static PlatformChar  k_SDKFileHelper_MACDELCHAR = (SDKFileHelper::isMacPosixPathEnabled()) ? k_SDKFileHelper_UNIXDELCHAR : k_SDKFileHelper_MACDELCHAR_HFS;
const static PlatformChar  k_SDKFileHelper_WINDELCHAR =('\\');
const static PlatformChar  k_SDKFileHelper_WIN_DRIVE_SEP =(':');
...
const static PMString   k_SDKFileHelper_MACDELIMITER ((SDKFileHelper::isMacPosixPathEnabled()) ? "/" : ":");
const static PMString   k_SDKFileHelper_UNIX_WINPARENTDIR ("..");
static const PMString 	k_SDKFileHelper_MACDELIMITER_HFS (":");
static const PMString 	k_SDKFileHelper_MACPARENTDIR_HFS ("::");
static const bool16 	k_SDKFileHelper_PosixEnabled = SDKFileHelper::isMacPosixPathEnabled();
const static PMString   k_SDKFileHelper_MACPARENTDIR = (k_SDKFileHelper_PosixEnabled) ? k_SDKFileHelper_UNIX_WINPARENTDIR : k_SDKFileHelper_MACPARENTDIR_HFS;
...
bool16 SDKFileHelper::isUnixPath(const PMString& path) const
{
	...
	return !(path.Contains(k_SDKFileHelper_WINDELIMITER)|| (path.Contains(k_SDKFileHelper_MACDELIMITER) && !k_SDKFileHelper_PosixEnabled) ||
			(path.Contains(k_SDKFileHelper_MACDELIMITER_HFS) && k_SDKFileHelper_PosixEnabled));
}
...
bool16 SDKFileHelper::isMacPath(const PMString& path) const
{
	...
	// one that is qualified by a directory)
	return !(path.Contains(k_SDKFileHelper_WINDELIMITER)|| (path.Contains(k_SDKFileHelper_UNIXDELIMITER) && !k_SDKFileHelper_PosixEnabled) ||
             (path.Contains(k_SDKFileHelper_MACDELIMITER_HFS) && k_SDKFileHelper_PosixEnabled));
}
...
bool16 SDKFileHelper::isWinPath(const PMString& path) const
{
	bool16 simpleCase = (path.Contains(k_SDKFileHelper_UNIXDELIMITER)|| (path.Contains(k_SDKFileHelper_MACDELIMITER))
		||(k_SDKFileHelper_PosixEnabled && path.Contains(k_SDKFileHelper_MACDELIMITER_HFS)));
	if(!simpleCase) {
		// No alien chars in sight; fine
		return kTrue;
	}
	// We might have c: and I don't want to discount this possibility
	if( path.CharCount()>2 && path[0] != k_SDKFileHelper_WIN_DRIVE_SEP  && path[1]== k_SDKFileHelper_WIN_DRIVE_SEP && path[2]==k_SDKFileHelper_WINDELCHAR) {
		return kTrue;
	}
...
bool16 SDKFileHelper::isAbsolutePath(const PMString& path) const
{
	...
	do {
		...
		if (this->isUnixPath(path) && this->isWinPath(path)) 
		{
			// two out of three ...
			retval = kFalse;
			break;
		}
		// check for unix path. This also takes care of mac-posix path
		...
		if ( (this->isWinPath(path)) && ( (path[0]==k_SDKFileHelper_WINDELCHAR) || 
													( path.CharCount()>2 && path[0] != k_SDKFileHelper_WIN_DRIVE_SEP
																				&& path[1]== k_SDKFileHelper_WIN_DRIVE_SEP
																				&& path[2]==k_SDKFileHelper_WINDELCHAR))) {
			retval = kTrue;
			break;
		}

	if ((this->isMacPath(path)) && (path[0] !=k_SDKFileHelper_MACDELCHAR) && !k_SDKFileHelper_PosixEnabled)  {
#ifdef MACINTOSH	
			// mac-hfs paths are different, they don't start with a  colon
			IDFile	theFile;
...
PMString SDKFileHelper::getMinusFirstElement(const PMString& path) const
{
	...
	do {
		...
		{	
			if (retval[0] == k_SDKFileHelper_MACDELCHAR && !k_SDKFileHelper_PosixEnabled) // we know we start with a "::" for HFS Path
			{
				retval.Remove(0,2);
				break;
			} 
...
PMString SDKFileHelper::getAsMacPath(const PMString& path) const
{
	...
	do {
		...
		if(!k_SDKFileHelper_PosixEnabled)
		{
		...
		for (int32 i = 0; i<retval.CharCount(); i++)
			if ((retval[i] == k_SDKFileHelper_UNIXDELCHAR && !k_SDKFileHelper_PosixEnabled) || (retval[i] == k_SDKFileHelper_WINDELCHAR) 
				|| (retval[i] == k_SDKFileHelper_MACDELCHAR_HFS && k_SDKFileHelper_PosixEnabled)) // want the mac delimeter
			{
				if(retval[i] != k_SDKFileHelper_MACDELCHAR_HFS && k_SDKFileHelper_PosixEnabled && i == 0)
                {
                    retval.Remove(i);
...
PMString SDKFileHelper::getAsWinPath(const PMString& path) const
{
	...
	do {
		...
		// If someone inputs mac HFS path, replace all instances of :: with .. 
		int32 position = retval.IndexOfString(k_SDKFileHelper_MACPARENTDIR_HFS);
		while (position!=(-1))
		{
			retval.Remove(position,2); // remove the "::"
			...
			position = retval.IndexOfString(k_SDKFileHelper_MACPARENTDIR_HFS);
		}
		...
		position = retval.IndexOfString(k_SDKFileHelper_MACDELIMITER_HFS);
		...
		if(haveReplacedDeviceChar) {
			...
			retval.Insert(k_SDKFileHelper_MACDELIMITER_HFS, devicePosition);
		}
		// now get the unix delimeter sorted.
		// mac posix path would also be handled here
