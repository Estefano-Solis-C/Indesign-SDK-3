...
// Project includes:
#include "BscDlgID.h"
#include "PublicTestHeader.h"
#include "OpenTestHeader.h"
...
/* CREATE_PMINTERFACE
 ...
 application.
*/
CREATE_PMINTERFACE(BscDlgActionComponent, kTestPrivateHeaderActionComponentImpl)
...
void BscDlgActionComponent::DoAction(IActiveContext* ac, ActionID actionID, GSysPoint mousePoint, IPMUnknown* widget)
{
	...
	switch (actionID.Get())
	{
		case kTestPrivateHeaderAboutActionID:
		{
			this->DoAbout();
			break;
		}	

		case kTestPrivateDialogDialogActionID:
		{
			this->DoDialog();
			break;
		}
...
void BscDlgActionComponent::DoDialog()
{
	do
	{
		...
		(
			...
			kTestPrivateHeaderPluginID,			// Our Plug-in ID from BasicDialogID.h. 
