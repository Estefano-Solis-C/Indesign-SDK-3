...
/* CREATE_PMINTERFACE
 ...
 application.
*/
CREATE_PMINTERFACE(BscDlgDialogObserver, kTestPrivateHeaderDialogObserverImpl)
...
void BscDlgDialogObserver::AutoAttach()
{
	...
	do
	{
		...
		AttachToWidget(kTestPrivateHeaderIconSuiteWidgetID, IID_ITRISTATECONTROLDATA, panelControlData);
...
void BscDlgDialogObserver::AutoDetach()
{
	...
	do
	{
		...
		DetachFromWidget(kTestPrivateHeaderIconSuiteWidgetID, IID_ITRISTATECONTROLDATA, panelControlData);
...
)
{
	...
	do
	{
		...
		if (theSelectedWidget == kTestPrivateHeaderIconSuiteWidgetID && theChange == kTrueStateMessage)
		{
			// About box
			CAlert::ModalAlert
			(
